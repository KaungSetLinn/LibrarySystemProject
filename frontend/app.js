 ...
 *
 *                【2026-05-12 改訂】開発メンバーリポジトリ統合版
 *                - SQLiteRepository を実 API 連動に置換
 *                - シークレット機能(secretMode)で豊田 v3.0.6 拡張を有効化可能
 *                - ON 時は開発メンバー API + localStorage ハイブリッドで動作
 *                - OFF 時は開発メンバー API のみで純粋構成案B として動作
 *
 * 作成者       : Y.Toyoda
 * 作成日       : 2026-04-17
 * 最終改訂     : 2026-05-12
 ...
 * 役割：データアクセスを抽象化する。
 *   - SQLiteRepository : 開発メンバーのバックエンド API を呼び出す(本番)
...
/* ----------------------------------------------------------------------------
 * SQLiteRepository
 *   開発メンバーの Sequelize ベース API を fetch で呼び出す本番リポジトリ。
 * -------------------------------------------------------------------------- */
const SQLiteRepository = (() => {
  ...
  // 予約 CRUD は開発メンバー backend に未実装。secretMode に関わらず "未実装" を返す。
  function reserveBook() {
    return Promise.resolve({
      success: false,
      message: "予約登録APIはバックエンド未実装です。開発メンバー backend での実装をお待ちください。"
    });
  }
  function cancelReservation() {
    return Promise.resolve({
      success: false,
      message: "予約取消APIはバックエンド未実装です。開発メンバー backend での実装をお待ちください。"
    });
  }
...
/* ----------------------------------------------------------------------------
 * HybridRepository
 *   SQLiteRepository(開発メンバー API)を主、LocalRepository を従として結合する
